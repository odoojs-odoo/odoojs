...
class List extends React.Component {
  ...
  columns = [
    {
      ...
      dataIndex: '_',
      render: (_, { id }) => {
        return <Link to={`/BackCompany/User?id=${id}`}>查看</Link>;
      },
    },
    {
      title: '名称',
  ...
  async componentDidMount() {
    const me0 = await odoo.me({
      company_id: { name: 1, company_registry: 1, user_id: 1, user_ids: 1 },
    });
    if (me0.id) {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Modal title="新建用户" visible={visible} onOk={this.handleOk} onCancel={this.handleCancel}>
          <Form>
